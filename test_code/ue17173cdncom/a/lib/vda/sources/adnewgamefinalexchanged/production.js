adv.AdBase.extend('AdNewgameFinalExchanged', {
  ...
  buildHtml: function(gameData, data) {
    return  '<div class="fix-sameip">'+
              '<div class="fix-sameip-c1">'+
                ...
                '</div>'+
                '<div class="btn-box">'+
                  '<a href="//newgame.17173.com/game-info-'+gameData.game_code+'.html" target="_blank" class="btn-c1">进入专区</a>'+
                  '<a href="//hao.17173.com/sche-info-'+gameData.game_code+'.html" target="_blank" class="btn-c2">礼包领取</a>'+
                '</div>'+
                '<a href="javascript:;"  class="btn-gload">游戏下载'+
                  '<span class="ewm">'+
                    '<img src="'+data.ewmLink+'" width="120" height="120" alt="" />'+
                  '</span>'+
                '</a>'+
                '<p class="intr">'+
                  '<a href="'+data.giftLink+'" target="_blank">'+gameData.game_intro+'</a>'+
                '</p>'+
              '</div>'+
            '</div>'+
  ...
  setup: function(data) {
    ...
    this.data = data;
      $.ajax({
        url: '//test.newgame.17173.com/api-gameDetailInfoJs-'+data.gameCode+'.html?callback=?',
        type: 'GET',
        dataType: 'jsonp',
        success:function(data1){
          if(data.pcGameCode == gameCode) {
    ...
    '.fix-sameip-close{position:absolute;top:-10px;right:-10px;z-index:10;width:25px;height:25px;background:url(//ue1.17173cdn.com/a/newgame/index/2016/fix-sameip/img/fix-sameip-sp.png) no-repeat 0 0}'+
    '.fix-sameip-close:hover{background-position:-30px 0}'+
    '.fix-sameip .btn-gload{position:relative;display:block;margin-bottom:10px;width:100%;height:32px;background:#ffcd00;color:#414141;text-align:center;font:700 16px/32px "Microsoft YaHei";}'+
    ...
    '.fix-sameip-c2{position:absolute;z-index:11;top:0;right:0;display:none;padding:5px 7px 35px;width:1pc;background:#ffcd00 url(//ue1.17173cdn.com/a/newgame/index/2016/fix-sameip/img/fix-sameip-sp.png) no-repeat -60px bottom;color:#333;font-family:"Microsoft YaHei";line-height:1.3;cursor:pointer}';

    if($(window).width() >( parseInt($('.content').width(),10) + 170) ) {
      this.css+='.fix-sameip{z-index:1000; background:#fff; position:fixed;top:314px;left:50%; margin-left:'+(parseInt($('.content').width()/2,10))+'px;width:170px;font: normal 14px/1.7 "Microsoft YaHei";}'+
      '.fix-sameip-c1{padding-top:29px;background: url(//ue3.17173cdn.com/a/newgame/index/2016/fix-sameip/img/fix-bg.png) no-repeat 2px top;}</style>';
    } else {
       this.css+='.fix-sameip{z-index:1000; background:#fff; position:fixed;top:314px;right:0; width:170px;font: normal 14px/1.7 "Microsoft YaHei";}'+
      '.fix-sameip-c1{padding-top:29px;background: url(//ue3.17173cdn.com/a/newgame/index/2016/fix-sameip/img/fix-bg.png) no-repeat 2px top;}</style>';
