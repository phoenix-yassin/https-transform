adv.AdBase.extend('AdNewgameNewserverList', {
    ...
    this.tmplFn10w = function(){/*@preserve
       <div class="mod-game mt35">
          ...
          <div class="info">
            ...
            <div class="links">
              <a href="@actUrl" target="_blank" >开服活动</a>
              <a href="@giftUrl" target="_blank" class="btn-gift">特权礼包</a>
    ...
    */}
        ...
        <div class="mod-top">
          <div class="mod-c1">
            <ul class="comm-plist comm-plist-ex2">
              <li>
                <i class="tag1"></i>
                <div class="tit"><a href="@link" target="_blank">@title<span class="tit-sub">@smalltitle</span></a></div>
                <div class="detail">
          ...
          <div class="mod-c2">
                 <embed src="@video" wmode="transparent" allowfullscreen="true" quality="high" width="446" height="288" align="middle" allowscriptaccess="always"  type="application/x-shockwave-flash"  autoplay="true" title="Adobe Flash Player">
        </div>
    ...
    // 投放3个30w的时候
     this.tmplFn30w2 = function() {/*@preserve
       <li>
            <i class="tag1"></i>
            <div class="tit"><a href="@link" target="_blank">@title</a></div>
            <div class="avatar">
              <a href="javascript:;"  data-video="@video" class="js-showvideo">
                <img src="@source" width="280" height="200" alt=""/>
                <i class="cover"></i>
     ...
     */}
     ...
     this.tmplFn30wfloat = function() {/*@preserve
         <a href="@link1" class="adfloat1 link" target="_blank" ><span>@text</span></a>
     */}
  ...
  if(val.adshowtype == '10w' ) {
        me.html10w = $(adv.razor(me.tmplFn10w,{link: val.link, source: val.source, actUrl: val.actUrl, giftUrl: val.giftUrl, gwUrl: val.gwUrl, registerUrl: val.registerUrl,
            openDate: val.openDate, serverList: val.serverList, title:val.title}));
        $('.'+me.options.advid).find('.pn-bd').append(me.html10w);
        //元素, 统计代码,是否发送曝光
        adv.util.sendIpa(me.html10w.find('.links a:eq(0)'),  val.adskey1, true);   //开服活动
        adv.util.sendIpa(me.html10w.find('.links a:eq(1)'),  val.adskey2, true);  //特权礼包
...
// 一个三十万
  if(val.adshowtype == '30w' && me.counts30w  == 1 ) {
     ...
     openDate: val.openDate, serverList: val.serverList, title:val.title, smalltitle:val.smalltitle, video: val.video}));
      $('.'+me.options.advid).find('.pn-bd').append(me.html30w);
      adv.util.sendIpa(me.html30w.find('.links a:eq(0)'),  val.adskey1, true); //开服活动
      adv.util.sendIpa(me.html30w.find('.links a:eq(1)'),  val.adskey2, true); //特权礼包
      ...
      $('.'+val.elmentid).prepend(me.html30wfloat);
...
// 两个三十万
  ...
  if(val.adshowtype == '30w' && me.counts30w  == 2 ) {
    me.html30w1 = $(adv.razor(me.tmplFn30w1,{ source: val.source, actUrl: val.actUrl, giftUrl: val.giftUrl, gwUrl: val.gwUrl,
    registerUrl: val.registerUrl,  openDate: val.openDate, serverList: val.serverList, title:val.title, smalltitle:val.smalltitle, video: val.video}));
  
   $('.'+me.options.advid).find('.pn-bd').find('.show30wtwo').append(me.html30w1);
    adv.util.sendIpa(me.html30w1.find('.links a:eq(0)'),  val.adskey1, true);  //开服活动1
    adv.util.sendIpa(me.html30w1.find('.links a:eq(1)'),  val.adskey2, true); //特权礼包1
  ...
  if( !$('.'+me.options.advid).find('.show30wthree').length && me.counts30w  == 3  ) {
   ...
   if(val.adshowtype == '30w' && me.counts30w  == 3 ) {
      me.html30w2= $(adv.razor(me.tmplFn30w2,{source: val.source, actUrl: val.actUrl, giftUrl: val.giftUrl, gwUrl: val.gwUrl,
      registerUrl: val.registerUrl,  openDate: val.openDate, serverList: val.serverList, title:val.title, smalltitle:val.smalltitle,  video: val.video}));
    ...
    me.html30wfloat = $(adv.razor(me.tmplFn30wfloat,{link1: val.link1, text: val.text}));
    $('.'+ val.elmentid).prepend(me.html30wfloat);
    adv.util.sendIpa(me.html30w2.find('.links a:eq(0)'),  val.adskey1, true); //开服活动1
    adv.util.sendIpa(me.html30w2.find('.links a:eq(1)'),  val.adskey2, true); //特权礼包1
  ...
  $('.AdNewgameNewserverList').on('click', '.js-showvideo', function() {
      ...
      '<embed src="'+videoUrl+'" allowfullscreeninteractive="true" quality="high" align="middle" allowscriptaccess="always" type="application/x-shockwave-flash" width="800" height="500" flashvars="m10=0"></div> </div>');
  })
