...
(function() {
    ...
    TopBackgroundAdv.prototype.play = function() {
      ...
      var a = this.getParam("hidebg"), container = $('#AdFirstBanner');
      if (!a) {
        ...
        if(this.options.topSrc.indexOf('.swf') >= 0){
          container.html('<embed type="application/x-shockwave-flash" width="'+this.options.topWidth+'" height="'+this.options.topHeight+'" bgcolor="#ffffff" quality="high" wmode="opaque" src="' + this.options.topSrc + '" />')
        } else {
    ...
    adv.AdBase.extend('AdNewsTopAndBg', {
